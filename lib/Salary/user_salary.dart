...
class UserInfoSalary extends StatefulWidget {
  final String userId;
  final String displayName;

  const UserInfoSalary({super.key, required this.userId, required this.displayName});

  @override
  State<UserInfoSalary> createState() => _UserInfoSalaryState();
}

class _UserInfoSalaryState extends State<UserInfoSalary> {
    ...
    try {
      ...
      if (user != null) {
        ...
        if (userSnapshot.exists) {
          setState(() {
            _userAccess = userSnapshot['access'] ?? [];
  ...
  Future<void> _loadSalaryDataForPeriod(int month, int year) async {
    try {
      ...
      final totalAbsentDays = totalWorkingDays - totalPresentDays - totalLeaveDays - publicHolidaysInPeriod;
  ...
  Widget build(BuildContext context) {
    ...
    if (!_userAccess.contains('Slip Gaji')) {
      return Scaffold(
        appBar: AppBar(
          title: Text(
            'Laporan Gaji: ${widget.displayName}',
        ...
        body: const NoAccessWidget(featureName: 'Slip Gaji'),
      );
    }
